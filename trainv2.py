...
from helpers.metricsv2 import get_losses, compute_metrics
...
class Trainer:
    ...
    def evaluate(self, writer=None, prefix="", verbose=False):
        ...
        for i, batch in enumerate(tqdm(eval_dataloader, desc="Evaluating")):
            ...
            if len(predictions['score'].shape) == 1:
                print(predictions['score'].shape, len(predictions['score'].shape))
                predictions['score'] = predictions['score'].reshape(1, predictions['score'].shape[0])

            if verbose or self.args.predictions_file:
                #print(predictions['score'].shape, len(predictions['score'].shape))
                if len(predictions['score'].shape) > 1:
                    all_score_predictions[start:end] = torch.argmax(predictions['score'].detach(), dim=1)
                else:
                    all_score_predictions[start:end] = torch.argmax(predictions['score'].detach(), dim=0)
                all_score_targets[start:end] = labels['score'].detach()
            #print("L129", predictions['score'].shape, labels['score'].shape)
    ...
    def _train_iteration(self, batch, scheduler, optimizer, step, tb_writer):
        ...
        if len(training_objective_predictions['score'].shape) == 1:
                training_objective_predictions['score'] = training_objective_predictions['score'].reshape(1, training_objective_predictions['score'].shape[0])
